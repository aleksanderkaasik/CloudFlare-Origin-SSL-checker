...
jq -r 'to_entries[].key' $workingDirectory/zoneConfig.json | while read -r zone; do
    zoneId=$(jq -r ".\"$zone\".id" $workingDirectory/zoneConfig.json )
    domain=$zone
    subDomains=$(jq -r ".\"$zone\".\"sub-domains\"[]" $workingDirectory/zoneConfig.json )
